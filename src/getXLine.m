...
if (~strcmp(selectionType, 'open'))
    ...
    GETLINE_X = [GETLINE_X FIXED_X];
    GETLINE_Y = [GETLINE_Y y];  % Keep X constant
...
% Clamp newy to axis limits
ylim = get(GETLINE_AX, 'ylim');
newy = max(min(newy, ylim(2)), ylim(1));

x = [GETLINE_X FIXED_X];
y = [GETLINE_Y newy];  % Keep X constant
